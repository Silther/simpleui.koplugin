...
-- ---------------------------------------------------------------------------
--
-- ## BOOKFUSION API — COMPLETE REFERENCE
-- (derived from bookfusion.koplugin/bf_api.lua, bf_browser.lua, bf_sync.lua,
--  bf_downloader.lua; keep this block in sync when those files change)
--
-- ### Transport
--   Base URL:      https://www.bookfusion.com
--   All endpoints: /api/user/...
--   Headers:
--     Accept:        application/json; api_version=10       (always)
--     Authorization: Bearer <access_token>                  (all except /auth/*)
--     Content-Type:  application/json                       (when body present)
--   JSON encoder/decoder: `rapidjson` (ships with KOReader).
--   Success codes: 200 and 201 are treated as OK.  Callers may whitelist
--   extra codes via `opts.expected_codes` (e.g. 404 for "no reading position",
--   204 for DELETE).  Network-layer failures (timeout, SSL handshake, sink
--   timeout) surface as ok=false, data={ error = "timeout" }.
--
-- ### Endpoints we call from SUI (and their exact contracts)
--
-- • POST /api/user/books/search                    api:searchBooks(params)
--     REQ body (all fields optional):
--       { page         = <int>,      -- 1-based
--         per_page     = <int>,      -- server default 20; bf_browser uses 20
--         sort         = <string>,   -- "last_read_at-desc", "added_at-desc",
--                                    -- "title-asc", etc.  Field-direction format.
--         query        = <string>,   -- full-text search
--         list         = <string>,   -- "currently_reading" | "planned_to_read"
--                                    --   | "completed" | "favorites"
--         bookshelf_id = <int>,      -- filter by custom bookshelf
--         series_id    = <int>,      -- filter by series
--       }
--     RESP body: Book[] (array)
--     RESP headers:  current-page, current-per-page, total-count
--       (api:searchBooks flattens these into a third return value:
--        `ok, books, { page = <n>, per_page = <n>, total = <n> }`)
--
-- • POST /api/user/bookshelves/search               api:searchBookshelves()
--     REQ body: {}
--     RESP body: Bookshelf[]   ({ id, name, … } per shelf)
--
-- • POST /api/user/series/search                    api:searchSeries(params)
--     Same shape as /books/search, returns Series[] with pagination headers.
--
-- • POST /api/user/books/<id>/download              api:getDownloadUrl(id, content_type?)
--     REQ body: { content_type = "epub" | "pdf" | ... } (optional)
--     RESP body: { url = <pre-signed https URL> }
--     ERR statuses worth handling: 403 (book not downloadable), 404 (not in library)
--
-- • GET  /api/user/books/<id>/reading_position     api:getReadingPosition(id)
--     Whitelisted 404 (no remote position yet → ok=true, data=nil).
--     RESP: { percentage = <0..100.0000>, page_position_in_book = <num>,
--             cfi = <string>, chapter_index = <int>, updated_at = <ISO 8601> }
--
-- • POST /api/user/books/<id>/reading_position     api:setReadingPosition(id, pos)
--     REQ body: position table (see Sync.getCurrentPosition below).
--     RESP: { updated_at = <ISO 8601> }
--
-- • POST /api/user/highlights                      api:createHighlight(h)
-- • POST /api/user/highlights/search               api:searchHighlights(book_id)
-- • PATCH /api/user/highlights/<id>                api:updateHighlight(id, patch)
-- • DELETE /api/user/highlights/<id>               api:deleteHighlight(id)  (204/404 ok)
--
-- • POST /api/user/reading/track                   api:trackReadingTime(records)
--     REQ body: { records = [ { book_id, duration_seconds, started_at, … } ] }
--
-- • POST /api/user/auth/device                     api:requestDeviceCode()
-- • POST /api/user/auth/token                      api:pollForToken(device_code)
--     OAuth 2.0 device-code grant; responses carry access_token, device_code,
--     user_code, verification_uri, expires_in, interval (for polling).
--
-- ### Book object shape (every item in /books/search response)
--   NOTE on nesting: the API returns an OBJECT for `cover`, not flat fields.
--   Bf_browser.lua:271-280 flattens to cover_url / cover_w / cover_h when
--   building Menu items — THIS PLUGIN does the same in Cache.put above.  A
--   v1 mistake was to assume flat fields on the raw response — don't repeat.
--
--   {
--     id             = <integer>,              -- used in download / sync URLs
--     title          = <string>,
--     authors        = Author[] | string | nil -- see Author shape below
--     cover          = { url, width, height } | nil   -- nested object
--     format         = "epub" | "pdf" | …      -- lowercase, no dot
--     percentage     = <0..1 float>,           -- reading progress fraction
--     download_size  = <bytes> | nil           -- used for download confirm
--     -- plus many other fields we don't consume (publisher, isbn, tags, etc.)
--   }
--
-- ### Author shape
--   Either a plain string ("John Doe") or an array of:
--     [ "String" | { name = <string>, id = <int>, … }, … ]
--   Always coerce via Downloader.formatAuthors() or our local _authorsLine().
--   Never table.concat(authors, ", ") directly — mixed types crash it.
--
-- ### Reading position shape (what we push, what we get back)
--   Paging document (PDF / DjVu):
--     { page_position_in_book = <num>,      -- document progress in book
--       percentage             = <0..100.0000> }   -- 4-decimal percent
--   Flowing document (EPUB):
--     { percentage             = <0..100.0000>,
--       cfi                    = <EPUB CFI string>,
--       chapter_index          = <1-based spine idx>,
--       page_position_in_book  = <float, spine-relative progress> }
--   The server keys the row by book_id; response carries updated_at (ISO 8601).
--
-- ### Highlight shape
--   REQ (createHighlight):
--     { book_id       = <int>,
--       quote_text    = <string>,
--       color         = "#RRGGBB",           -- COLOR_TO_HEX mapping in bf_sync
--       chapter_title = <string> | nil,
--       chapter_index = <int>,                -- 0 for paging docs
--       added_at      = <datetime string>,
--       note          = <string> | nil,
--       -- EPUB only:
--       start_offset  = <int>,                -- UTF-16 char offset
--       end_offset    = <int>,
--       -- Paging only:
--       page          = <int> }
--   RESP: { id = <int>, … }  (server assigns the id)
--   searchHighlights RESP:
--     Highlight[] with id, chapter_index, start_offset, end_offset,
--     quote_text, color, note, added_at, …
--
-- ### Pagination rules (applied by our Data.fetchListAll)
--   - Ask for `per_page` up to 50 (bigger than bf_browser's 20 → fewer RTs).
--   - A page with < per_page items is the last page.
--   - If server sets total-count and #accumulated >= total, stop.
--   - Hard safety belt at page 200 so bugs can't spin forever.
-- ===========================================================================
